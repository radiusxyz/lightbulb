...
impl AuctionWorker {
    ...
    async fn process_auctions(&self) -> Result<(), String> {
        ...
        let mut auctions = self.manager.auctions.write().await;

        for (_chain_id, chain_auctions) in auctions.iter_mut() {
            for (_auction_id, auction_state) in chain_auctions.iter_mut() {
